...
//		s = s.Trim().Trim("unionall".ToCharArray());
//		Debug.Log(s);
	...
	*/
	...
	public List<string> AllCode (string file_name, ref bool open_flag) 
	{
		...
		{
			for(int i = 0; i < all_code_temp.Count; i++)
			{
				temp_str_list = new List<string>();
				temp_str_list = CodeFormat(all_code_temp[i]);
...
//		sourceCode = sourceCode.ToUpper().Trim().TrimStart(';', '；', '%').TrimEnd(';', '；', '%');
		...
		*/
		...
		{//1 level
			...
			{//2 level
				...
				//--7-- /\d? 匹配代码跳过符号 “/”或者“/1-9”
				//--8-- ;?  匹配";"，为了适应在程序中插入代码的情况
				string pattern_macro = @"([A-Z]+[^A-Z^\s^#^;^=^\[^\]]+)|([A-Z]+#\d+)|([A-Z]+\[[^\[\]]*(((?'Open'\[)[^\[\]]*)+((?'-Open'\])[^\[\]]*)+)*(?(Open)(?!))\])|(#\d+)|(#\[[^\[\]]*(((?'Open'\[)[^\[\]]*)+((?'-Open'\])[^\[\]]*)+)*(?(Open)(?!))\])|(=\S+$)|(/\d?)|(;{1})";
				Regex macro_test_Reg = new Regex(pattern_macro, RegexOptions.IgnoreCase);
...
//				Regex format_normal_Reg = new Regex(@"([A-Z]+[^A-Z^\s^;]+) | (;{1})", RegexOptions.IgnoreCase);
				Regex format_normal_Reg = new Regex(@"([A-Z]+[^A-Z^\s^;]+)|(;{1})", RegexOptions.IgnoreCase);
//				([A-Z]+[^A-Z^\s^#^;^=^\[^\]]+)|(;{1})
				MatchCollection format_normal_Mat = format_normal_Reg.Matches(sourceCode);
				if(format_normal_Mat.Count > 1)
				{
					int str_index = 0;
					string temp_str = "";
					for(int i = 0; i < format_normal_Mat.Count; i++)
					{
						temp_str = format_normal_Mat[i].Value;
						code_segment.Add(temp_str);
						str_index = sourceCode.IndexOfAny(temp_str.ToCharArray());
						sourceCode = sourceCode.Remove(str_index, temp_str.Length);
					}
